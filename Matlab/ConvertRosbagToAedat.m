...
function aedat_struct = ConvertRosbagToAedat(filename_rosbag)

% Load a bag and get information about it
bag = ros.Bag.load(filename_rosbag);
bag.info()

% Start to fill in aedat structure, for conversion
aedat_struct.importParams = [];
aedat_struct.importParams.filePath = filename_rosbag;
aedat_struct.data = [];
...
if (num_msgs > 0)
    aedat_struct.data.frame = []; % create images struct
    aedat_struct.data.frame.numEvents = num_msgs; % number of frames
    ...
    aedat_struct.data.frame.xPosition = uint16(v_zero);
    aedat_struct.data.frame.yPosition = uint16(v_zero);
    aedat_struct.data.frame.xLength = uint16(img_width * v_one); % 240
    aedat_struct.data.frame.yLength = uint16(img_height* v_one); % 180
    for ii = 1:num_msgs
        ...
        aedat_struct.data.frame.timeStampStart(ii) = uint32(time_us);
        aedat_struct.data.frame.timeStampEnd(ii) = uint32(time_us);
        ...
        aedat_struct.data.frame.samples{ii,1} = img; % cell array with the grayscale values
    end
    % column vectors
    aedat_struct.data.frame.timeStampStart = aedat_struct.data.frame.timeStampStart';
    aedat_struct.data.frame.timeStampEnd = aedat_struct.data.frame.timeStampEnd';
    disp('...done!');
end
...
if (num_msgs > 0)
    aedat_struct.data.imu6 = []; % create IMU struct
    aedat_struct.data.imu6.numEvents = num_msgs; % number of measurements
    disp('Converting IMU messages...');
    % Column vectors
    v_zero = zeros(num_msgs,1);
    aedat_struct.data.imu6.timeStamp = uint32(v_zero);
    aedat_struct.data.imu6.accelX = v_zero;
    aedat_struct.data.imu6.accelY = v_zero;
    aedat_struct.data.imu6.accelZ = v_zero;
    aedat_struct.data.imu6.gyroX = v_zero;
    aedat_struct.data.imu6.gyroY = v_zero;
    aedat_struct.data.imu6.gyroZ = v_zero;
    aedat_struct.data.imu6.temperature = 28 + v_zero; % centigrades
    for ii = 1:num_msgs
        % timestamp: remove offset, convert to microseconds and cast to uint32
        time_us = double(msgs{ii}.header.stamp.sec - time_first_sec_uint32) * 1e6 ...
            + double(msgs{ii}.header.stamp.nsec) / 1e3; % microseconds
        aedat_struct.data.imu6.timeStamp(ii) = uint32(time_us);
        
        aedat_struct.data.imu6.accelX(ii) = msgs{ii}.linear_acceleration(1);
        aedat_struct.data.imu6.accelY(ii) = msgs{ii}.linear_acceleration(2);
        aedat_struct.data.imu6.accelZ(ii) = msgs{ii}.linear_acceleration(3);
        aedat_struct.data.imu6.gyroX(ii) = msgs{ii}.angular_velocity(1);
        aedat_struct.data.imu6.gyroY(ii) = msgs{ii}.angular_velocity(2);
        aedat_struct.data.imu6.gyroZ(ii) = msgs{ii}.angular_velocity(3);
        % aedat_struct.data.imu6.temperature(ii) = 0;
    end
    
    % Conversion of units.
    % Acceleration is imported as m/s^2, and aedat wants it in multiples of g
    gravity = 9.8; % m/s^2
    aedat_struct.data.imu6.accelX = aedat_struct.data.imu6.accelX / gravity;
    aedat_struct.data.imu6.accelY = aedat_struct.data.imu6.accelY / gravity;
    aedat_struct.data.imu6.accelZ = aedat_struct.data.imu6.accelZ / gravity;
    % Angular velocity is imported as rad/s, and aedat expects deg/s
    rad_in_deg = 180 / pi; % degrees in one radian
    aedat_struct.data.imu6.gyroX = aedat_struct.data.imu6.gyroX * rad_in_deg;
    aedat_struct.data.imu6.gyroY = aedat_struct.data.imu6.gyroY * rad_in_deg;
    aedat_struct.data.imu6.gyroZ = aedat_struct.data.imu6.gyroZ * rad_in_deg;
...
if (num_msgs > 0)
    aedat_struct.data.polarity = []; % create events struct
    ...
    aedat_struct.data.polarity.numEvents = numEvents;
    ...
    for ii = 1:num_msgs
        ...
        aedat_struct.data.polarity.timeStamp(idx) = uint32(time_us);
        
        if (ev_sensor_width == 128) || (ev_sensor_width == 346)
            % DVS and miniDAVIS346 need to flip events. Don't ask why
            aedat_struct.data.polarity.x(idx) = uint16(ev_sensor_width-1 -msgs{ii}.events(1,:));
        else
            aedat_struct.data.polarity.x(idx) = uint16(msgs{ii}.events(1,:));
        end
        aedat_struct.data.polarity.y(idx) = uint16(ev_sensor_height-1 -msgs{ii}.events(2,:));
        aedat_struct.data.polarity.polarity(idx) = logical(msgs{ii}.events(4,:));
    end
    % column vectors
    aedat_struct.data.polarity.timeStamp = aedat_struct.data.polarity.timeStamp';
    aedat_struct.data.polarity.x = aedat_struct.data.polarity.x';
    aedat_struct.data.polarity.y = aedat_struct.data.polarity.y';
    aedat_struct.data.polarity.polarity = aedat_struct.data.polarity.polarity';
    disp('...done!');
end
...
if exist('ev_sensor_width','var') && exist('ev_sensor_height','var')
    ...
    aedat_struct.info.deviceAddressSpace = [ev_sensor_width, ev_sensor_height];
elseif exist('img_width','var') && exist('img_height','var')
    % Get the sensor size from the image topic
    aedat_struct.info.deviceAddressSpace = [img_width, img_height];
else
...
sensor_size = aedat_struct.info.deviceAddressSpace;
if (sensor_size(1) == 240) && (sensor_size(2) == 180)
    aedat_struct.info.source = 'Davis240C';
elseif (sensor_size(1) == 346) && (sensor_size(2) == 260)
    aedat_struct.info.source = 'Davis346BMono';
elseif (sensor_size(1) == 128) && (sensor_size(2) == 128)
    aedat_struct.info.source = 'Dvs128';
else
    error(['Unknown sensor type for resolution: ' ...
        num2str(sensor_size(1)) ' x ' num2str(sensor_size(2)) ' pixels']);
end

% aedat_struct.info.numEventsInFile = ;
% aedat_struct.info.firstTimeStamp = ;
% aedat_struct.info.lastTimeStamp = ;


%% Write output to aedat file
aedat_struct.exportParams = [];
aedat_struct.exportParams.filePath = [filename_rosbag(1:end-4) '.aedat'];
ExportAedat2(aedat_struct);
