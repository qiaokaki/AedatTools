...
accelX = input.data.imu6.accelX; % conversion from g to full scale, and shift bits
accelX = int16(accelX * 8192); % conversion from g to full scale 16 range
accelX = [accelX zeros(input.data.imu6.numEvents, 1, 'int16')];
accelX = accelX';
accelX = accelX(:);
...
accelY = input.data.imu6.accelY; % conversion from g to full scale, and shift bits
accelY = int16(accelY * 8192); % conversion from g to full scale 16 range
accelY = [accelY zeros(input.data.imu6.numEvents, 1, 'int16')];
accelY = accelY';
accelY = accelY(:);
...
accelZ = input.data.imu6.accelZ; % conversion from g to full scale, and shift bits
accelZ = int16(accelZ * 8192); % conversion from g to full scale 16 range
accelZ = [accelZ zeros(input.data.imu6.numEvents, 1, 'int16')];
accelZ = accelZ';
accelZ = accelZ(:);
...
temp = input.data.imu6.temperature; % conversion from g to full scale, and shift bits
temp = int16((temp - 35) * 340); % conversion from K to full scale 16 range
temp = [temp zeros(input.data.imu6.numEvents, 1, 'int16')];
temp = temp';
temp = temp(:);
...
gyroX = input.data.imu6.gyroX; % conversion from g to full scale, and shift bits
gyroX = int16(gyroX * 65.5); % conversion from g to full scale 16 range
gyroX = [gyroX zeros(input.data.imu6.numEvents, 1, 'int16')];
gyroX = gyroX';
gyroX = gyroX(:);
...
gyroY = input.data.imu6.gyroY; % conversion from g to full scale, and shift bits
gyroY = int16(gyroY * 65.5); % conversion from g to full scale 16 range
gyroY = [gyroY zeros(input.data.imu6.numEvents, 1, 'int16')];
gyroY = gyroY';
gyroY = gyroY(:);
...
gyroZ = input.data.imu6.gyroZ; % conversion from g to full scale, and shift bits
gyroZ = int16(gyroZ * 65.5); % conversion from g to full scale 16 range
gyroZ = [gyroZ zeros(input.data.imu6.numEvents, 1, 'int16')];
gyroZ = gyroZ';
gyroZ = gyroZ(:);
...
timeStamps = uint32(input.data.imu6.timeStamp(:));
timeStamps = repmat(timeStamps', 7 , 1);
timeStamps = timeStamps(:);

output = zeros(1, 2 * input.data.imu6.numEvents * 7, 'uint32'); % allocate horizontal vector to hold output data
